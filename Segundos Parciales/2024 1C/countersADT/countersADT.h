...
* Se asume que la gran mayoría de los mostradores serán utilizados por las aerolíneas. * Si maxCounters es cero retorna NULL.
*/
countersADT newCounters(size_t maxCounters);

/* Ingresa al pasajero #passenger en la cola de espera del mostrador #counterNumber * para luego poder hacer el checkIn de su vuelo.
* Un mismo pasajero puede ingresar múltiples veces al mismo mostrador o a varios * mostradores distintos.
...
/* Funciones de iteración para poder consultar para un mostrador los pasajeros que están * esperando, en ORDEN DE LLEGADA (ver ejemplo en programa de prueba)
*/
...
/* Para cada mostrador, realiza el checkIn del pasajero que está esperando primero en la * cola, si es que hay al menos un pasajero esperando en ese mostrador.
